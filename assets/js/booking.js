...
function changeTimeslot() {
    ...
    var raw = JSON.stringify({
        "query": "SELECT ts.bookingTime FROM (SELECT t.bookingTime, w.washingMachineID FROM laundrotech.Booking AS b, laundrotech.WashingMachine AS w, laundrotech.Timeslot AS t " +
    ...
    fetch("https://3rczj928aa.execute-api.us-east-1.amazonaws.com/prod/gettimeslot", requestOptions)
        ...
        .then(result => {
            ...
            if (data.length < 1) {
                raw = JSON.stringify({ "query": "SELECT bookingTime FROM laundrotech.Timeslot" });
                requestOptions = {
                    method: 'POST',
                    headers: myHeaders,
                    body: raw,
...
function displayTable() {
    ...
    var raw = JSON.stringify({
        "query": "SELECT wm.washingMachineID, wm.washingMachineBrand, wm.washingMachineModel, wm.price, wm.washingMachineAddress FROM " +
    ...
    fetch("https://3rczj928aa.execute-api.us-east-1.amazonaws.com/prod/getavailwm", requestOptions)
        ...
        .then(result => {
            ...
            if (data.length < 1) {
                raw = JSON.stringify({ "query": "SELECT * FROM laundrotech.WashingMachine WHERE location = '" + sessionStorage.getItem("location") + "'" });
                requestOptions = {
                    method: 'POST',
                    headers: myHeaders,
                    body: raw,
